module ProductSourcesHelper
    def provider_badge_color(provider_type)
      case provider_type
      when 'lenta' then 'bg-blue-100 text-blue-800'
      when 'magnit' then 'bg-green-100 text-green-800'
      when 'dixy' then 'bg-purple-100 text-purple-800'
      when 'perekrestok' then 'bg-yellow-100 text-yellow-800'
